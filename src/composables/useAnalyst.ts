...
import { lifePathDescriptions, summary, zodiacCompatibility } from '@/constants/meanings'
...
type UseAnalyst = {
  ...
  summarybyNumber: ComputedRef<string>
...
export const useAnalyst = (): UseAnalyst => {
  ...
  const compatibleZodiac = computed<string>(() => {
    ...
    return (
      zodiacCompatibility[lifePathNumber.value] ?? 'Không thể xác định cung hoàng đạo tương thích.'
    )
  })
  //Summary
  const summarybyNumber = computed<string>(() => {
    if (!lifePathNumber.value) return ''
    return summary[lifePathNumber.value]
  })
  return {
    birthdate,
    error,
